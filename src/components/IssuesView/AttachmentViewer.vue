<template>
  <div class="content-fulied">
    <b-navbar type="dark" variant="success">
      <b-navbar-brand to="/issueedit">&lt;&lt; 指摘 編集</b-navbar-brand>
    </b-navbar>
...
export default {
  ...
  computed: {
    ...
    contentUrlSrc () {
      let url
      if (this.attachment.attachment !== null) {
        url = URL.createObjectURL(this.attachment.attachment.file)
      } else {
        url = URLjoin(config.BaseURL, '/data/', editstate.issue.id + '/', this.attachment.id + '_' + this.attachment.filename)
      }
      return url
    },
    contentUrlSrcset () {
      ...
      if (this.attachment.attachment !== null) {
        ...
        url = URLjoin(config.BaseURL, '/data/', editstate.issue.id + '/', this.attachment.id + '_' + this.attachment.filename + ' 1x')
      }
      return url
    }
  },
